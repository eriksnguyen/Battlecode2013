...
public class Soldier extends MyRobot {
	...
	private static void findClosestType(Robot[] robots, RobotType r)
			throws GameActionException {
		int mod;
		switch(r){
		case SHIELDS: mod = 1; break;
		case MEDBAY: mod = 2; break;
		default: mod = 0;
		}
		int broadCast = rc.readBroadcast(getChannel(Clock.getRoundNum(), mod));
	...
	private static void beAttacker(boolean defense, boolean getShield)
		...
		if (enemyRobots.length == 0) { // No nearby enemies
			...
			if (getShield)
				findClosestType(
						rc.senseNearbyGameObjects(Robot.class, 100000, myTeam),
						RobotType.SHIELDS);
			if (getShield
					&& closestShield != null
					&& distToShield < distToEnemy_2
					&& info.shields < 100 * coefficientOfSuperman
					&& rc.senseNearbyGameObjects(Robot.class, shieldLoc, 2,
							myTeam).length < 9)
				moveTo(shieldLoc);
	...
	private static MapLocation setEncampmentTarget(MapLocation prevTarget)
		...
		if (ret != null) {// About 60 bytecode
			setEncampmentType(ret.distanceSquaredTo(allyHQ),
					ret.distanceSquaredTo(enemyHQ));
	...
	private static void setEncampmentType(int dist_to_aHQ_2, int dist_to_eHQ_2)
		...
		if (dist_to_eHQ_2 < Math.min(HQSeparation / 2, 63))
			...
			findClosestType(allyRobots, RobotType.SHIELDS);
			if (closestShield == null)
				encampmentType = RobotType.SHIELDS;
			else
				encampmentType = Math.random() < .1 ? RobotType.SHIELDS
						: RobotType.ARTILLERY;
