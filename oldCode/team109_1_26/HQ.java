...
public class HQ extends MyRobot {
	...
	public static MapLocation[] mainVector;
	...
	public static void run(RobotController myRc) {
		...
		while (true) {
			try {
				...
				if (notYet && Clock.getRoundNum() == 2) {
					notYet = false;
					mainVector = getMainVector();
				}
				...
				} else if (rc.senseNearbyGameObjects(Robot.class, 1000000, rc
						.getTeam().opponent()).length < 3) {
					rc.broadcast(channel + 1, 3);// Send soldiers into
													// field-mine mode
				}
	...
	private static Upgrade[] upgrades = new Upgrade[] { Upgrade.FUSION,
			Upgrade.PICKAXE, Upgrade.DEFUSION, Upgrade.VISION, Upgrade.NUKE };
	...
	private static MapLocation[] getMainVector() {
		...
		while ((prev.x - enemyHQ.x) * (prev.x - enemyHQ.x)
				+ (prev.y - enemyHQ.y) * (prev.y - enemyHQ.y) > 2) {
			list.add(prev);
